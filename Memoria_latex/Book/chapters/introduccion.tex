...
En este contexto, el presente trabajo tiene como objetivo evaluar la capacidad de distintos modelos de procesamiento del lenguaje natural para detectar automáticamente indicadores de ansiedad, depresión y \ac{TCA} en textos escritos en español. Para ello, se realiza una comparación entre enfoques basados en modelos consolidados y modelos de lenguaje de última generación.

Con el fin de alcanzar este objetivo general, se plantean los siguientes objetivos específicos:

\begin{itemize}
	\item Analizar y preprocesar un conjunto de datos en español que contenga textos anotados con la presencia o ausencia de ansiedad, depresión y trastornos de la conducta alimentaria.
	\item Entrenar y evaluar modelos basados en \acs{BERT} mediante técnicas de \textit{fine-tuning} para tareas de clasificación.
	\item Aplicar \acp{LLM}, tanto de código abierto como propietarios, mediante estrategias de \textit{zero-shot prompting} utilizando \acp{API} de distintos proveedores, con el fin de predecir la presencia de los distintos trastornos sin necesidad de reentrenamiento.
	\item Comparar los resultados obtenidos por ambos enfoques, analizando distintas métricas estándar de evaluación.
	\item Analizar las ventajas, limitaciones y posibles sesgos derivados del uso de modelos de lenguaje en tareas relacionadas con la salud mental.
\end{itemize}

En este contexto, el presente trabajo tiene como objetivo evaluar y comparar la capacidad de distintos modelos de procesamiento del lenguaje natural para la detección automática de indicadores de salud mental en textos escritos en español, centrándose en ansiedad, depresión y trastornos de la conducta alimentaria (\ac{TCA}). Para ello, se realiza una comparación entre modelos encoder tradicionales basados en \ac{BERT}, \acp{LLM} open-source y \acp{LLM} propietarios de última generación.
...
\begin{itemize}
	...
	\item Plantear tanto tareas de clasificación binaria independientes para ansiedad, depresión y \ac{TCA}, como una tarea de clasificación multiclase. Las tareas binarias permiten evaluar la capacidad de los modelos para detectar la presencia o ausencia de cada trastorno de forma individual, mientras que la tarea multiclase permite analizar su capacidad para distinguir simultáneamente entre usuarios control, ansiedad, depresión y \ac{TCA}, así como identificar qué clases tienden a confundirse entre sí.
	
	\item Entrenar y evaluar modelos encoder basados en \acs{BERT} mediante técnicas de \textit{fine-tuning}, incluyendo arquitecturas preentrenadas tanto específicamente para español como modelos multilingües y de contexto largo.
	...
	\item Comparar de forma rigurosa ambos enfoques utilizando métricas de rendimiento, estabilidad predictiva, análisis de errores mediante matrices de confusión, comparación estadística mediante bootstrap pareado y análisis de significancia.
	
	\item Evaluar la relación entre rendimiento y coste computacional de los distintos modelos, analizando su viabilidad práctica en escenarios reales de detección temprana en salud mental.
	
	\item Identificar ventajas, limitaciones y posibles implicaciones del uso de modelos de lenguaje aplicados a tareas de cribado automático en salud mental.
