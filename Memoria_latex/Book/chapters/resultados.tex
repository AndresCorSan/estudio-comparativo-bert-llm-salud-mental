...
Dado que comparar todos los modelos entre sí generaría un número excesivo de combinaciones y dificultaría la interpretación, se seleccionaron únicamente los modelos más representativos de cada grupo: el mejor modelo BERT, el mejor LLM open-source y los dos mejores LLMs propietarios. Además, cada comparación pareada se realizó únicamente sobre el subconjunto común de ejemplos para los que ambos modelos disponían de predicción\footnote{Esto se debe a que los modelos BERT fueron evaluados únicamente sobre el conjunto de test (15\% del dataset original), mientras que los \acp{LLM} generaron predicciones sobre el conjunto completo. Por ejemplo, en la tarea de ansiedad los modelos BERT disponían de predicciones para 75 ejemplos, frente a los 500 ejemplos clasificados por los \acp{LLM}. Por tanto, las comparaciones estadísticas se realizaron únicamente sobre esos 75 ejemplos compartidos, garantizando así una evaluación homogénea entre ambos paradigmas.}, garantizando así una evaluación homogénea y directamente comparable entre sistemas.
...
En conjunto, estos resultados indican que, aunque los LLMs muestran mejores métricas agregadas que los modelos BERT, en la tarea de ansiedad las diferencias entre los mejores modelos no siempre son suficientemente robustas desde el punto de vista estadístico.
...
\begin{table}[H]
	...
	\begin{tabular}{llllll}
		...
		\hline
		RoBERTa-large-BNE & DeepSeek-V3.2 & -0.013 [-0.111, 0.093] & -0.248 & Small & 0.832 \\
		RoBERTa-large-BNE & Gemini 3.1 Pro & -0.015 [-0.120, 0.096] & -0.237 & Small & 0.832 \\
		RoBERTa-large-BNE & GPT-5.2 & 0.028 [-0.071, 0.141] & 0.541 & Medium & 0.832 \\
		DeepSeek-V3.2 & Gemini 3.1 Pro & -0.027 [-0.066, 0.008] & -1.422 & Large & 0.486 \\
		DeepSeek-V3.2 & GPT-5.2 & 0.008 [-0.030, 0.044] & 0.434 & Small & 0.832 \\
		Gemini 3.1 Pro & GPT-5.2 & 0.036 [0.005, 0.064] & 2.316 & Large & 0.180 \\
		\hline
	\end{tabular}
...
La Tabla~\ref{tab:bootstrap_depression} muestra que, en la tarea de depresión, las diferencias entre los modelos con mejor rendimiento son relativamente pequeñas y no alcanzan significancia estadística tras aplicar la corrección de Benjamini-Hochberg. Todos los \textit{p-values} ajustados permanecen por encima del umbral convencional de 0.05, indicando que las diferencias observadas en F1-score macro podrían explicarse por la variabilidad del conjunto de prueba.

Las comparaciones que involucran a RoBERTa-large-BNE reflejan un comportamiento especialmente competitivo del mejor modelo BERT frente a los \acp{LLM}. Las diferencias frente a DeepSeek-V3.2 y Gemini 3.1 Pro son prácticamente nulas (\(\Delta = -0.013\) y \(\Delta = -0.015\), respectivamente), mientras que frente a GPT-5.2 incluso aparece una ligera ventaja positiva para RoBERTa-large-BNE (\(\Delta = 0.028\)). Sin embargo, en todos los casos los intervalos de confianza incluyen el cero, por lo que no existe evidencia estadística suficiente para afirmar diferencias robustas entre estos modelos.

Entre los \acp{LLM}, Gemini 3.1 Pro presenta la mayor diferencia media frente a GPT-5.2 (\(\Delta = 0.036 \ [0.005,\ 0.064]\)), acompañada además de un tamaño del efecto elevado (\(d = 2.316\)). No obstante, esta diferencia tampoco alcanza significancia estadística tras la corrección \ac{BH} (\(p = 0.180\)). De forma similar, las comparaciones entre DeepSeek-V3.2 y el resto de modelos muestran diferencias reducidas y estadísticamente no concluyentes. En conjunto, estos resultados sugieren que los mejores modelos evaluados presentan un rendimiento muy similar en la tarea de depresión, sin que pueda afirmarse una superioridad estadísticamente significativa entre ellos.
...
\begin{table}[H]
	...
	\begin{tabular}{llllll}
		...
		\textbf{Modelo A} & \textbf{Modelo B} & \textbf{Mean $\Delta$} & \textbf{Cohen's d} & \textbf{Effect size} & \textbf{\textit{p}} \\
		\hline
		RoBERTa-large-BNE & GPT-OSS-20B & -0.020 [-0.106, 0.039] & -0.561 & Medium & 0.640 \\
		RoBERTa-large-BNE & GPT-5.2 & -0.040 [-0.148, 0.000] & -1.410 & Large & 0.420 \\
		RoBERTa-large-BNE & Gemini 3.1 Pro & -0.020 [-0.103, 0.039] & -0.567 & Medium & 0.640 \\
		GPT-OSS-20B & GPT-5.2 & -0.031 [-0.063, -0.006] & -2.152 & Large & 0.108 \\
		GPT-OSS-20B & Gemini 3.1 Pro & -0.018 [-0.048, 0.009] & -1.313 & Large & 0.396 \\
		GPT-5.2 & Gemini 3.1 Pro & 0.012 [-0.003, 0.032] & 1.430 & Large & 0.282 \\
		\hline
	\end{tabular}
...
Los resultados de la Tabla~\ref{tab:bootstrap_tca} muestran que las diferencias entre los mejores modelos en la tarea de \ac{TCA} son muy reducidas, confirmando el comportamiento ya observado previamente en la Tabla~\ref{tab:resultados_tca_metricas}. Ninguna de las comparaciones alcanza significancia estadística tras aplicar la corrección de Benjamini-Hochberg, ya que todos los \textit{p-values} ajustados permanecen por encima de 0.05.

Las comparaciones que involucran a RoBERTa-large-BNE reflejan nuevamente el comportamiento altamente competitivo del mejor modelo BERT frente a los \acp{LLM}. Las diferencias frente a GPT-OSS-20B y Gemini 3.1 Pro son prácticamente idénticas (\(\Delta = -0.020\)), mientras que frente a GPT-5.2 aparece la mayor diferencia observada (\(\Delta = -0.040\)), sugiriendo una ligera ventaja del modelo propietario. Sin embargo, todos los intervalos de confianza incluyen el cero, por lo que no existe evidencia estadística suficiente para afirmar diferencias robustas entre estos sistemas.

Entre los \acp{LLM}, la comparación más favorable para GPT-5.2 frente a GPT-OSS-20B muestra una diferencia negativa de \(\Delta = -0.031 \ [-0.063,\ -0.006]\), acompañada además de un tamaño del efecto elevado (\(d = -2.152\)). Aun así, tras la corrección \ac{BH} el resultado no alcanza significancia estadística (\(p = 0.108\)). Del mismo modo, las comparaciones restantes entre modelos generativos presentan diferencias pequeñas y estadísticamente no concluyentes. En conjunto, estos resultados sugieren que, en la tarea de clasificación de \ac{TCA}, tanto los mejores modelos BERT como los \acp{LLM} alcanzan niveles de rendimiento muy próximos, sin que puedan identificarse ventajas estadísticamente significativas de unos sobre otros.
...
Aunque algunas comparaciones presentan valores de Cohen’s \(d\) clasificados como \textit{Medium}, la amplitud de los intervalos de confianza y los elevados \textit{p-values} impiden consolidar estas diferencias como robustas desde el punto de vista estadístico. Esto indica que, en el escenario multiclase, al igual que ocurría en las tareas de clasificación binaria, varios modelos presentan un comportamiento muy similar, dificultando establecer una jerarquía clara entre ellos a partir del conjunto de evaluación disponible.
